...
impl orml_nft::Config for Runtime {
	...
	type ClassData = fantour_nft::ClassData<BlockNumber>;
	type TokenData = fantour_nft::TokenData<BlockNumber>;
}
...
parameter_types! {
	...
	pub const NftModuleId: ModuleId = ModuleId(*b"fantour*");
}

impl fantour_nft::Config for Runtime {
	type Event = Event;
...
impl fantour_core::Config for Runtime {
	type Event = Event;
}
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Fantour: fantour_nft::{Module, Call, Storage, Event<T>, Config<T>},
		Config: fantour_core::{Module, Call, Storage, Event<T>},
	}
);
...
impl_runtime_apis! {
	...
	impl fantour_rpc_runtime_api::FantourApi<Block> for Runtime {
		fn mint_token_deposit(metadata_len: u32, quantity: u32) -> (Balance, Balance) {
			Fantour::mint_token_deposit(metadata_len, quantity)
		}
		fn add_class_admin_deposit(admin_count: u32) -> Balance {
			Fantour::add_class_admin_deposit(admin_count)
		}
		fn create_class_deposit(metadata_len: u32, name_len: u32, description_len: u32) -> (Balance, Balance) {
			Fantour::create_class_deposit(metadata_len, name_len, description_len)
		}
	}
