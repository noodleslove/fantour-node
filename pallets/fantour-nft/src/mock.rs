...
impl orml_nft::Config for Runtime {
	...
	type ClassData = fantour_nft::ClassData<BlockNumberOf<Self>>;
	type TokenData = fantour_nft::TokenData<BlockNumberOf<Self>>;
}
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		FantourCore: fantour_core::{Module, Call, Event<T>},
		Fantour: fantour_nft::{Module, Call, Event<T>},
	}
);
...
impl ExtBuilder {
	pub fn build(self) -> sp_io::TestExternalities {
		...
		ext.execute_with(|| {
			...
			FantourCore::add_whitelist(Origin::root(), ALICE);
			FantourCore::add_whitelist(Origin::root(), BOB);
		});
		ext
	}
}
